...
def index(request):
	...
	if request.user.is_authenticated():
		return render(request, 'index.html', {'Autenticado':'Si', 'usuario' : request.user.username, 'font' : font, 'color':color} )
	
	return render(request, 'index.html', {'Autenticado':'No', 'usuario' : request.user.username, 'productos':producto, 'font' : font, 'color':color})
...
def contactenos(request):
	...
	if request.user.is_authenticated():
		return render(request, 'contact.html', {'Autenticado':'Si' , 'font':font, 'color' : color})
	return render(request, 'contact.html', {'Autenticado':'No','font':font, 'color' : color})
...
def registrar(request):
	...
	if request.user.is_authenticated():
		return render(request, 'formulario_crear_cliente_landing.html', {'Autenticado':'Si', 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font})
	return render(request, 'formulario_crear_cliente_landing.html', {'Autenticado':'No', 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font})
...
def carrito(request):
	...
	if request.user.is_authenticated(): 
		...
		total = 2000+subtotal
		option = False  
		context = {'Autenticado':'Si','carrito':canasta,'option':option,'total':total, 'subtotal':subtotal,'font':font, 'color' : color} 
        ...
        option = False  
        context = {'Autenticado':'No','message':message,'carrito':canasta,'option':option,'total':total, 'subtotal':subtotal,'font':font, 'color' : color}
        return render_to_response( 'checkout.html',context, context_instance=RequestContext(request) )
        
        
	return render(request, 'checkout.html', {'Autenticado':'No', 'font':font, 'color' : color})
...
class cuenta(TemplateView):
	...
	def get(self,request,*args,**kwargs):
		...
		if request.user.is_authenticated():
			usuario = Usuario.objects.get(id=request.user.id)
			return render(request, 'extra_profile.html', {'usuario':usuario, 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font})
...
def modificar_cliente_landing(request):
	...
	return render(request, 'formulario_modificar_cliente_landing.html', {'usuario':usuario, 'tarjeta_credito' : usuarioCliente.tarjeta_credito, 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font})
	
def loginIn(request):

	if request.method == 'POST':
		if "emailRecuperar" in request.POST:
			try:
				email = request.POST['emailRecuperar']
				print email
				usuario = Usuario.objects.get(correo_electronico=email)
				print "bandera 1"
				correo = "La contraseña para tu cuenta de Pizzeria,  "+ str(email)+", se ha cambiado recientemente. Si la has cambiado tú, no necesitas realizar ninguna otra acción. Si no la has cambiado tú, es posible que tu cuenta haya sido vulnerada. Tu nueva contraseña es:   "
				longitud = 15
				...
				usuario.save()
				print "bandera 4"
				correo2 = correo + str(passw) + "  Es recomendable cambiar la contraseña de recuperación por una propia en cuanto accedas a la cuenta."
				asunto = "Recuperación de contraseña de Pizzería"
				print "bandera 5"
				send_mail(asunto, correo2 , settings.EMAIL_HOST_USER, [email], fail_silently=False)
				print "bandera 6"
				mensaje = {'status':'True'}
			except Exception, e:
				print str(e)		
...
class CrearUsuarioAdministrador(TemplateView):
    ...
    def get(self,request,*args,**kwargs):
		...
		return render_to_response('formulario_crear_administrador.html',{'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font},
...
class CrearUsuarioDigitador(TemplateView):
    ...
    def get(self,request,*args,**kwargs):
		...
		return render_to_response('formulario_crear_digitador.html',{'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font},
    			context_instance=RequestContext(request))
...
class CrearUsuarioCliente(TemplateView):
    ...
    def get(self,request,*args,**kwargs):
    	...
    	return render_to_response('formulario_crear_cliente.html',{'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font},
	    		context_instance=RequestContext(request))
...
def modificar_usuario_view(request,id_usuario,clase_form,modelo):
	...
	return render(request, 'formulario_modificar_administrador.html', {'form':form,'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font})
...
class ModificarUsuario(TemplateView):
    ...
    def get(self,request,*args,**kwargs):
    	...
    	if kwargs['modelo'] == 'Digitador':
    		return render_to_response('formulario_modificar_digitador.html',{'usuario':usuario, 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font},
    			context_instance=RequestContext(request))
    			
    	elif kwargs['modelo'] == 'Administrador':
    		return render_to_response('formulario_modificar_administrador.html',{'usuario':usuario, 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font},
    			context_instance=RequestContext(request))
    	else:
    		usuarioCliente = Cliente.objects.get(id=id)
    		return render_to_response('formulario_modificar_cliente.html',{'usuario':usuario, 'tarjeta_credito': usuarioCliente.tarjeta_credito, 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font},
    			context_instance=RequestContext(request))
...
def visualizar_usuario_view(request,id_usuario,modelo):
	...
	return render(request, 'perfil_usuario.html', {'usuario':usuario , 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font})
...
def listar_usuario_view(request):
	...
	return render(request, 'listar_usuarios.html', {'lista':lista, 'black': tinte, 'grid' : sidebar_grid, 'inverse': nav_var_color, 'color' : color, 'font' : font})
...
#@login_required()	
class DesactivarCliente(TemplateView):
	def post(self,request,*args,**kwargs):
		if "codigoUsuario" in request.POST:
			try:
			    id_usuario = request.POST['codigoUsuario']
			    usuario = get_object_or_404(Usuario,id = id_usuario)
			    usuario.estado = 'INACTIVO'
			    usuario.is_active=False
			    usuario.save()
			    mensaje = {'status':'True','codigoUsuario':id_usuario}			
			except:
				mensaje = {'status':'False'} 
			response = simplejson.dumps(mensaje)
			return HttpResponse(response ,content_type='application/json')
...
class ActivarCliente(TemplateView):
	def post(self,request,*args,**kwargs):
		if "codigoUsuario2" in request.POST:
			try:
			    id_usuario = request.POST['codigoUsuario2']
			    usuario = get_object_or_404(Usuario,id = id_usuario)
			    usuario.estado = 'ACTIVO'
			    usuario.is_active=True
			    usuario.save()
			    mensaje = {'status':'True','codigoUsuario2':id_usuario}			
			except:
				mensaje = {'status':'False'} 
			response = simplejson.dumps(mensaje)
			return HttpResponse(response ,content_type='application/json')
...
def killCliente(request):
	...
	if request.user.is_authenticated():
		...
		dicc={}
		dicc['nombre_usuario'] = request.user.username
		dicc['fecha_nacimiento'] = str(usuario.fecha_nacimiento)
		dicc['numero_identificacion'] = usuario.numero_documento_identificacion
		dicc['correo_electronico'] = usuario.correo_electronico
		dicc['tipo_documento'] = usuario.tipo_documento_identificacion
		dicc['telefono'] = usuario.telefono
		dicc['primer_nombre'] = usuario.primer_nombre
		dicc['segundo_nombre'] = usuario.segundo_nombre
		dicc['primer_apellido'] = usuario.primer_apellido
		dicc['segundo_apellido'] = usuario.segundo_apellido
		dicc['tarjeta_credito'] = cliente.tarjeta_credito
		dicc['fecha_registro'] = str(cliente.fecha_registro)
		
		
		data=json.dumps(dicc)
		    
		zip.writestr("info_cliente-"+ usuario.correo_electronico +".txt", data)
		...
		cliente = Cliente.objects.get(id  = user_id).delete()
...
class RecuperarContra(TemplateView):
	def post(self,request,*args,**kwargs):
		try:
			email = request.POST['email']
			usuario = Usuario.objects.get(correo_electronico=email)
			correo = "La contraseña para tu cuenta de Pizzeria,  correo@correo.com, se ha cambiado recientemente. Si la has cambiado tú, no necesitas realizar ninguna otra acción.	Si no la has cambiado tú, es posible que tu cuenta haya sido vulnerada. Tu nueva contraseña es: "
			passw = random.randint(1000000000, 9999999999)
			usuario.set_password(passw)
			usuario.save()
			correo += passw +" ."
			asunto = "Recuperación de contraseña de Pizzería"
			send_mail(asunto, correo , settings.EMAIL_HOST_USER, [email], fail_silently=False)
		except:
